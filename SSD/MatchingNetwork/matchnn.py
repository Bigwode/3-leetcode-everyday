...
class MatchCosine(_Merge):
    ...
    def call(self, inputs):
        ...
        the target embedding as the second last value in the array,
        and true class of target embedding as the last value in the array
        """ 
        similarities = []
        ...
        for ii in range(numsupportset):
            ...
            # reciprocal of the magnitude of the member of the support
            ...
            # reciprocal of the magnitude of the query image

            dot_product = tf.matmul(tf.expand_dims(targetembedding,1),
                                    tf.expand_dims(supportembedding,2))
...
class MatchEuclidean(_Merge):
    ...
    def build(self, input_shape):
        if not isinstance(input_shape, list) or len(input_shape) != self.nway+2:
            raise ValueError('A ModelEuclidean layer should be called on a list of '
                             'inputs of length %d'%(self.nway+2))
    ...
    def call(self,inputs):
        ...
        for ii in range(numsupportset):
            ...
            dd = tf.negative(tf.sqrt(tf.reduce_sum(
                tf.square(supportembedding-targetembedding),1,keep_dims=True)))
