...
class OmniglotNShotDataset():
    def __init__(self,batch_size,classes_per_set=5,samples_per_class=1,
                 trainsize=32000,valsize=10000):
    ...
    def packslice(self, data_pack, numsamples):
        ...
        ready to be fed to our networks
        """
        ...
        for iiii in range(numsamples):
            ...
            for j, cur_class in enumerate(classes):  # 对于挑选出来的5个support class
                ...
                if j == x_hat_class:
                    slice_x[n_samples,:,:,:] = \
                        data_pack[cur_class][np.random.choice(data_pack.shape[1])]  # 从当前类中随机找一个样本
                    target_y = j
