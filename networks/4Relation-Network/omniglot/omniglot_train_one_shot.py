...
class RelationNetwork(nn.Module):
    def __init__(self, input_size, hidden_size):  # [64,8]
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        # should add this two line in train phase.
        ...
        sample_dataloader = tg.get_data_loader(
            task,num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False,rotation=degrees)
        batch_dataloader = tg.get_data_loader(
            task,num_per_class=BATCH_NUM_PER_CLASS,split="test",shuffle=True,rotation=degrees)

        # sample datas
        samples, sample_labels = sample_dataloader.__iter__().next()  # [5,1,28,28],[5]
        batches, batch_labels = batch_dataloader.__iter__().next()  # [95,1,28,28], [95]
        ...
        # [475, 128, 5, 5]
        relations = relation_network(relation_pairs).view(-1,CLASS_NUM)  # [95, 5]

        mse = nn.MSELoss().cuda(GPU)
        one_hot_labels = Variable(torch.zeros(BATCH_NUM_PER_CLASS*CLASS_NUM, CLASS_NUM).scatter_(
            1, batch_labels.view(-1,1), 1)).cuda(GPU)
        ...
        if (episode+1)%2000 == 0:
            ...
            for i in range(TEST_EPISODE):
                # should add this two line in val phase.
                feature_encoder.eval()
                relation_network.eval()
                degrees = random.choice([0,90,180,270])
                task = tg.OmniglotTask(metatest_character_folders,
                                       CLASS_NUM,SAMPLE_NUM_PER_CLASS,SAMPLE_NUM_PER_CLASS,)
                sample_dataloader = tg.get_data_loader(task,
                    num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False,rotation=degrees)
                test_dataloader = tg.get_data_loader(task,
                    num_per_class=SAMPLE_NUM_PER_CLASS,split="test",shuffle=True,rotation=degrees)
