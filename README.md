...
（2）循环遍历数组，如果当前值和下标不同：先判断当前值和以当前值为坐标的位置上的值是不是相等，如果相等，就说明是重复的数字。
否则，交换当前值和以当前值为下标的数值。
...
思路：显式调用栈（或者使用vector.insert()函数直接在数组的begin()位置上直接插入），或者使用递归。

**7-0 树的遍历：前序遍历；中序遍历；后序遍历；宽度优先遍历** 的递归和非递归实现（××××××重点）
...
在pre中找到第一个元素，在in中找这个元素所在的位置，将pre和in分隔开。再递归调用函数分别建立左子树和右子树。
...
思路：不使用递归的做法，使用动态规划，从下往上计算。

**问题10-拓展1 青蛙跳台阶** 变态青蛙跳台阶：一次可以跳1-n个台阶。可以通过推导求出总次数为2^n-1
...
需要考虑的情况：当旋转的次数为0，即为原数组；或者是头指针和尾指针以及中间位置的数字相等的时候，就不能再继续用二分法，需要按照顺序依次查找最小的值。

**问题12：回溯法：判断矩阵中过是否有满足一个字符串输入的路径**

思路：需要设置一个数组记录元素是否被访问过。

然后判断是否当前值是否与字符串中当前位置的值相等，如果相等，就从它的上下左右中找下一个元素是否在字符串中，没有就会退一个格。

**问题13：机器人的运动范围**

方法同12，不同的是不需要回溯，使用递归计算。
...
思路：1、动态规划：子问题得到了最优解，总的问题也就是最优解。其实就是按照顺序，从小到大挨个试，将当前结果的最大值保存在数组中。总的时间复杂度O(n2)和空间复杂度为O(n)。

2、贪婪算法：先尽量分割成3大小的，如果剩下4，就分割成2,2大小的。

**问题15：统计二进制中1的个数**

考虑到可能是负数，负数右移左侧补的是符号位，会造成死循环。所有要左移无符号数1，直到1变为0

惊喜的解法：一个整数-1与该整数按位与操作，会使得该整数最右侧的1变为0
...
需要考虑：要删除的节点不是尾节点;要删除的节点是唯一节点;//要删除的节点是尾节点
...
*str=='\0' && *pattern!='\0'不能加是因为（“”， “.*”）

先判断下一个字符是不是*号，如果是，看当前字符是否匹配（直接匹配或者是正则字符为.的情况），

匹配上的时候， 可以匹配0个，或者循环实现多个匹配，或者只匹配一个。递归实现。
...
（1）、不考虑相对位置的情况，直接使用两个指针指向第一个元素和最后一个元素，或者使用下标前移和后移。判断奇数偶数考虑用&
...
注意k可能比链表长度还要大的情况，注意在循环中判断

**23、链表中环的入口节点**

首先应该判断链表中是否有环，通过设置快慢指针来判断。如果有环，返回相遇的节点。相遇的节点一定是处在环内的。之后计算环的长度m，即从相遇的节点开始转一圈，统计个数。最后，从头结点开始，快指针先走m步，慢指针再开始走，直到两个指针相遇，即为入口节点。

**24、翻转列表**

为保证指针改变之后，链表不断开，需要保存当前节点之后的节点。
...
思路２：统计数字及其出现次数。如果下一个数字等于上一个数字，次数＋１，否则，次数－１，次数等于０的时候，统计下一个数。最后留下来的数字一定是出现次数大于数组长度一半的数字。
...
思路１：类似于快速排序，使用Partion函数，找到index=k的时候前k个数即为最小的ｋ个数。时间复杂度：O(n)

思路２：先创建一个k大小的容器，依次比较删除和插入。可以利用最大堆和红黑树实现。
...
前半部分使用最大堆，后半部分使用最小堆。当两个堆中的总数目为偶数的时候，把新数据加入最小堆，否则插入最大堆。如果此时，新数据比最大堆的最大值要小，就把最大堆的最大值弹出来插入最小堆，把最新值插入最大堆。当新数据应插入最堆的时候，同理，需要判断和最小堆的最小值之间的大小关系。

**42、连续子数组的最大和**

思路：需要设置两个变量用来存放当前的和和和的最大值。当当前和小于０的时候，和改变为当前值。否则加上当前值，比较当前和和最大和大小。判断是否更新最大和。
